// Dimenson less para.: math, angle in rad
const double pi = TMath::Pi();
const double alpha = 1. / 137; // fine structure constant
const double alphapi = alpha / pi;
const double theta0_degr = 23.;
const double theta0 = theta0_degr * pi / 180.; // theta0 in radius
const double phase_angle = 155. / 180. * pi; // 155 or 163 phi-omega interference angle [degr] -> [rad]
const double me = 0.551; // electron mass in MeV/c²
const double crx_scale = 0.3894e6; // 1 [GeV]^{-2} = 0.3894 mb = 0.3894e6 nb
...
const double M_omega = 782; // omega mass
...
double GetSigma_V(double BB, double M_V) {// Calculate the peak cross section

  double M_V2 = M_V * M_V;

  double Sigma_V = 12. * pi * BB / M_V2 * 1e6 * crx_scale;

  return Sigma_V;
}

// Cross section in nb
const double Sigma_phi = GetSigma_V(BB_phi, M_phi); // Calculate phi peak cross section
const double Sigma_omega = 1457.; // omega peak cross section nb
const double Sigma_bkg = 12.; // back ground cross section nb, refer to https://arxiv.org/pdf/hep-ex/0002017.pdf; 12 +/- 5 nb


void SetGFAttr(TGraph *gf, TString x_title = "", TString y_title = "Lumi_{ISR} [nb^{-1}]") {
