...
vari_indx=0 # Variable index: 0: chi2 1: angle 2: deltaE 3: beta
step_nb=2 # Number of variations (one side)
sample_type=norm #norm, mini, chain
...
VARI=("chi2_cut" "angle_cut" "deltaE_cut" "beta_cut") # Labels
NORM=($chi2_cut $angle_cut $deltaE_cut $beta_cut) # Norminial value
STEP_SIZE=(1 0.67 2.24 0.0027) # Variation step size
...
echo "Variable: ${variable}, range [$vari_min, $vari_max], step size: $vari_step, # variations (one side): $step_nb"

## Check syst. input folder
#norm_path=../../result/input_${sample_type}_TDATA
norm_path=../../input_${sample_type}_TDATA
