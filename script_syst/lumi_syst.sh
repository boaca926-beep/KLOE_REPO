...
## Start loop
echo "Lumi_tot nominal=$Lumi_tot"
...
do min=$(echo "($vari_min+$j*$vari_step)" | bc)
   ...
   ## Variations
   indx=$(echo "$j+1" | bc)
   lumi=$(echo "${Lumi_tot}*(1+0.003*$min)" | bc)
   
   sed -i 's/\(const double Lumi_tot =\)\(.*\)/\1 '$lumi';/' $sm_header
   sed -i 's/\(const TString cut_nm =\)\(.*\)/\1 "'$variable'";/' $cut_header
   sed -i 's/\(double cut_value =\)\(.*\)/\1 '$min';/' $cut_header

   echo "index=$indx; $Lumi_tot; $variable=$lumi; variation=$min"
