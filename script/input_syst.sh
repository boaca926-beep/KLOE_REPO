...
#sample_path=../../path_${sample_size}/ 
syst_type=trigger # trigger filfo
...
result_path=../../input_${syst_type}_${sample_size}_${exp_type}
...
#sed -i 's/\(cut_value =\)\(.*\)/\1 '$cut_value';/' $cut_header
...
for ((i=0;i<${#DATA_TYPE[@]};++i)); do
    ...
    echo '  gROOT->ProcessLine(".L ../run_'${syst_type}'/MyClass.C");' >> $run_script
