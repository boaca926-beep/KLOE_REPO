...
vari_indx=1 # Variable index: 0: mass_sigma_nb 1: sfw2d_sigma_nb
...
VARI=("mass_sigma_nb" "sfw2d_sigma_nb") # Labels
NORM=($mass_sigma_nb $sfw2d_sigma_nb) # Norminial value
STEP_SIZE=(0.05 0.05) # Variation step size
...
do min=$(echo "$vari_min+$j*$vari_step" | bc)
   ...
   sed -i 's/\(const double '$variable' =\)\(.*\)/\1 '$min';/' $hist_header
